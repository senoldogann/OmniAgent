...
Kalıcı skill'ler `~/Library/Application Support/OmniAgent/skills/<ad>/SKILL.md` altında kurulur; önceden var olan `~/.agents/skills/<ad>/SKILL.md` ve `~/.codex/skills/<ad>/SKILL.md` dosyaları da bulunur. `skills.sh` kaynaklı kurulumu açıkça istediğinizde ajan depodaki ilgili dizini GitHub commit kimliğine sabitleyerek indirir, boyut ve dosya yolu sınırlarını denetler, ardından atomik olarak kaydeder. Skill içindeki kurulum betikleri çalıştırılmaz. Aynı adla başka bir kaynak zaten kuruluysa mevcut kayıt korunur. Sonraki sohbetler kurulu skill'i yerel katalogdan bulur. Kataloğa eklenen MCP paketleri `~/Library/Application Support/OmniAgent/packages/<paket-özeti>/` içinde ayrı ortamda kurulur; kayıtları `catalog.json` içinde kalır. Uzak HTTP MCP için paket indirilmez, katalog kaydı sonraki sohbette yeniden bağlanmak için yeterlidir. Masaüstü composer'ında ve Telegram'da `/tools` veya `/yetenekler` kayıtlı entegrasyonları ve skill sayısını, `/skills` skill adlarını model ya da ağ çağrısı olmadan gösterir. Belirli bir yeteneği kullanmak için hedefe adını yazın: `Context7 kullanarak React 19 API belgesine bak` gibi. Kurmak için `skills.sh/acme/repo/skill adresindeki skill'i indir ve kur` şeklinde açık hedef verin. Composer'da `@` özel seçim sözdizimi değildir. Yeni, incelenmemiş MCP/plugin, yalnız keşif sonucunda görünür; otomatik güvenilir sayılmaz.
